...
export function useCategoryForm(existing?: Category) {
  ...
  const form = useForm<CategoryFormDTO>({
    ...
    defaultValues: {
      ...
      budget: existing ? existing.budget.toString() : '',
    },
  });
  ...
  const onSubmit = form.handleSubmit((dto) => {
    ...
    if (existing) {
      updateCategory(existing.id, { name: dto.name, color: dto.color, icon: dto.icon, budget });
      router.back();
      showMessage({ message: 'Category updated', type: 'success' });
    } else {
      addCategory({ name: dto.name, color: dto.color, icon: dto.icon, budget });
      router.back();
